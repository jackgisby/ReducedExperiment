...
wave2_se <- readRDS("../data/wave1.rds")
...
stability_res <- estimate_stability(wave1_se, seed=1, n_runs=30, min_components=10, max_components=60, by=2, mean_stability_threshold=0.85)
...
wave1_fe <- estimate_factors(wave1_se, nc=35, seed=1, use_stability=TRUE, 
                             n_runs=30, stability_threshold=0.25)
...
```{r factor_alignments}
getAlignedFeatures(wave1_fe, format = "data.frame")
```
...
factor 2, that was downregulated in COVID-19 patients, and is enriched for XX.

```{r example_factor_enrichment}
ggplot(
    factor_enrich[factor_enrich$component == "factor_2" & factor_enrich$p.adjust < 0.05 ,],
    aes(-log10(p.adjust), reorder(ID, -log10(p.adjust)), fill = estimate)
) +
    geom_point(pch = 21, size = 3) +
    xlab("-log10(p-value)") +
...
ggplot(
    module_enrich[module_enrich$component == "module_2" & module_enrich$p.adjust < 0.05 ,],
    aes(-log10(p.adjust), reorder(ID, -log10(p.adjust)))
) +
    geom_point(pch = 21, size = 3) +
    xlab("-log10(p-value)") +
    ylab("Pathway name (ordered by p-value)") +
...
wave2_me <- calcEigengenes(wave1_me, wave2_me)
wave2_me
```
